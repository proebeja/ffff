...
def schreibe_mastersheet(wb: Workbook, konten, perioden, ergebnisse) -> int:
    ...
    for r, no in enumerate(sorted(konten), start=2):
        k = konten[no]
        e = ergebnisse[no]
        for i, wert in enumerate([k.konto, k.bezeichnung, k.gruppe, k.seite,
                                  e["klasse"], e["category"], e["wc_seite"],
                                  e["source"], e["rule_id"],
                                  "ja" if e["review"] else ""], start=1):
            ws.cell(r, i, wert).font = _schrift()
        for i, p in enumerate(perioden):
            _zahlzelle(ws, r, MS_ERSTE_PERIODE + i, round(k.saldo(p), 2), BLAU,
                       '#,##0.00;(#,##0.00);"-"')
    ws.freeze_panes = "B2"
